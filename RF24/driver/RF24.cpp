...
/* SPI Configuration we want
 * SPI MSB FIRST
 * SPI MODE 0 = CPOL = 0, CPHA = 0
 * clock speed = If we assume 2Mbs data rate and 16Mhz clock, a divider of 4 
 * is the minimum we want.
 * */
SPI SPIdevice(SPI_MOSI, SPI_MISO, SPI_SCLK);
DigitalOut ce_pin(SPI_CS);
DigitalOut csn_pin(RF_CSN);
...
{
    SPIdevice.lock(); //Specific to Mbed
    csn(LOW);
    // status = SPI.transfer( R_REGISTER | ( REGISTER_MASK & reg ) );
    uint8_t status = SPIdevice.write(R_REGISTER | ( REGISTER_MASK & reg ));
    while ( len-- )
    // *buf++ = SPI.transfer(0xff);
    *buf++ = SPIdevice.write(0xff);

    csn(HIGH);
    SPIdevice.unlock(); //Specific to Mbed
...
uint8_t RF24::read_register(uint8_t reg)
{
    SPIdevice.lock(); //Specific to Mbed
    csn(LOW);
    SPIdevice.write(R_REGISTER | ( REGISTER_MASK & reg ));
    uint8_t result = SPIdevice.write(0xff);
    csn(HIGH);
    SPIdevice.unlock(); //Specific to Mbed
...
uint8_t RF24::write_register(uint8_t reg, const uint8_t* buf, uint8_t len)
{
    SPIdevice.lock(); //Specific to Mbed
    csn(LOW);
    uint8_t status = SPIdevice.write( W_REGISTER | ( REGISTER_MASK & reg ) );
    while ( len-- )
    SPIdevice.write(*buf++);
...
uint8_t RF24::write_register(uint8_t reg, uint8_t value)
{
    ...
    SPIdevice.lock(); //Specific to Mbed
    csn(LOW);
    status = SPIdevice.write( W_REGISTER | ( REGISTER_MASK & reg ) );
    SPIdevice.write(value);
    csn(HIGH);
    SPIdevice.unlock(); //Specific to Mbed
...
uint8_t RF24::write_payload(const void* buf, uint8_t len)
{
    ...
    SPIdevice.lock(); //Specific to Mbed
    csn(LOW);
    status = SPIdevice.write( W_TX_PAYLOAD );
    while ( data_len-- )
    SPIdevice.write(*current++);
    while ( blank_len-- )
    SPIdevice.write(0);
    csn(HIGH);
    SPIdevice.unlock(); //Specific to Mbed
...
uint8_t RF24::read_payload(void* buf, uint8_t len)
    ...
    SPIdevice.lock(); //Specific to Mbed
    csn(LOW);
    status = SPIdevice.write( R_RX_PAYLOAD );
    while ( data_len-- )
    *current++ = SPIdevice.write(0xff);
    while ( blank_len-- )
    SPIdevice.write(0xff);
    csn(HIGH);
    SPIdevice.unlock(); //Specific to Mbed
...
uint8_t RF24::flush_rx(void)
{
    ...
    SPIdevice.lock(); //Specific to Mbed
    csn(LOW);
    status = SPIdevice.write( FLUSH_RX );
    csn(HIGH);
    SPIdevice.unlock(); //Specific to Mbed
...
uint8_t RF24::flush_tx(void)
{
    ...
    SPIdevice.lock(); //Specific to Mbed
    csn(LOW);
    status = SPIdevice.write( FLUSH_TX );
    csn(HIGH);
    SPIdevice.unlock(); //Specific to Mbed
...
bool RF24::write( const void* buf, uint8_t len )
{
    ...
    uint8_t observe_tx;
    uint8_t status;
    uint32_t sent_at = (us_ticker_read() / 100000.0f) ; //for Arduino use millis();
    ...
    while( ! ( status & ( _BV(TX_DS) | _BV(MAX_RT) ) ) && 
            ( (us_ticker_read() / 100000.0f) - sent_at < timeout ) );
...
uint8_t RF24::getDynamicPayloadSize(void)
{
  ...
  csn(LOW);
  SPIdevice.write( R_RX_PL_WID );
  result = SPIdevice.write(0xff);
  csn(HIGH);
...
{
  csn(LOW);
  SPIdevice.write( ACTIVATE );
  SPIdevice.write( 0x73 );
  csn(HIGH);
}
...
void RF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  csn(LOW);
  SPIdevice.write( W_ACK_PAYLOAD | ( pipe & 0b111 ) );
  const uint8_t max_payload_size = 32;
